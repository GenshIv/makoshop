...
## v1.5 — Брендинг: финализация

### Добавлено

- **Система брендирования** (украшения страницы под разные случаи):
  - 8 слотов: под хедером/над футером (полная ширина), баннер главной, баннер категории, 4 боковых
  - Наборы (`BrandSet`): имя, приоритет, вкл/выкл одним переключателем, картинки light/dark, ссылка по клику, regex-паттерны страниц
  - Переопределения по категориям (`BrandCategoryTheme`)
  - API: `GET /branding/active` (публичный, с версией для кэша) + админ CRUD
  - Админка `/admin/branding`: наборы с живым toggle, редактор слотов с загрузкой картинок и валидацией regex, вкладка переопределений
- **Ручное управление текстом главной страницы** (`/admin/settings` → «Текст главной страницы»):
  - Пер-языковые (ru/ua/en/pl) тексты hero-блока: заголовок, подзаголовок, слоган
  - Пустое поле = текст по умолчанию (i18n); лимит 300 символов на поле
  - Хранение: `global_settings.home_hero`

### Изменено

- Баннер главной (`home_banner`) теперь рендерится **внутри** hero-блока (`.hero-glow`), без отступов — сам баннер и есть блок (без padding/градиента, картинка на всю ширину, высота = её aspect ratio)
- Если включён баннер брендирования — текст hero скрывается (баннер заменяет его)
- `internal/api/settings_handlers.go`: `GET/PATCH /admin/settings` включает `home_hero` (валидация: 400 при превышении лимита)
- `frontend/src/composables/useSettings.js`: загрузка `home_hero`
- `frontend/src/views/CatalogView.vue`: `heroText` computed (ручной текст с fallback на i18n), баннер внутри hero
- `frontend/src/views/admin/AdminSettingsView.vue`: редактор текста hero (4 языка × 3 поля)
- i18n: `admin.home_hero_*` (ru/ua/en/pl)
