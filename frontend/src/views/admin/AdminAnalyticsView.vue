...
<template>
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
    <h1 class="text-2xl font-bold mb-6 text-purple-700">Аналитика</h1>
    ...
    <div v-else class="space-y-6">
      ...
      <div v-if="overview" class="bg-white rounded-lg shadow-sm p-4">
        <h2 class="font-bold mb-3">Обзор</h2>
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
          <div><div class="text-gray-500">Пользователи</div><div class="font-bold">{{ overview.users || 0 }}</div></div>
          <div><div class="text-gray-500">Товары</div><div class="font-bold">{{ overview.products || 0 }}</div></div>
          <div><div class="text-gray-500">Заказы</div><div class="font-bold">{{ overview.orders || 0 }}</div></div>
          <div><div class="text-gray-500">Выручка</div><div class="font-bold text-green-600">{{ formatPrice(overview.revenue || 0) }}</div></div>
        </div>
      </div>

      <!-- Orders analytics -->
      <div v-if="ordersAnalytics" class="bg-white rounded-lg shadow-sm p-4">
        <h2 class="font-bold mb-3">Заказы</h2>
      ...
      <div class="bg-white rounded-lg shadow-sm p-4">
        <h2 class="font-bold mb-3">Популярные поисковые запросы</h2>
        ...
        <div v-else class="text-sm">
          <div v-for="q in searchQueries.slice(0, 10)" :key="q.query" class="flex justify-between py-1 border-b last:border-b-0">
            <span>{{ q.query }}</span>
            <span class="text-gray-500">{{ q.count }} раз</span>
          </div>
        </div>
      </div>
    </div>
