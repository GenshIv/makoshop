...
func (h *Handlers) handleSCUPageCatalog(w http.ResponseWriter, r *http.Request, catID int64) {
	...
	if catID > 0 && h.attrDefRepo != nil {
		...
		for _, code := range codes {
			...
			if len(values) > 0 {
				categoryAttrs = append(categoryAttrs, map[string]interface{}{
					"code":    code,
					"options": values,
				})
			}
...
func (h *Handlers) writeSCUPageResponse(w http.ResponseWriter, r *http.Request, sp *model.SCUPage) {
	...
	// Build category tree path for SEO
	var treePath []string
	if sp.CategoryID != 0 {
...
func (h *Handlers) HandleAdminRebuildSCUPages(w http.ResponseWriter, r *http.Request) {
	...
	for scu, products := range scuProducts {
		...
		if err == nil && existing != nil {
			...
			if err := h.scuPageRepo.Create(sp); err != nil {
	...
	fmt.Printf("[REBUILD-SCUPAGES] SCU pages: %d created, %d updated, %d errors\n", created, updated, errors)
