// headload — нагрузочный тест через HEAD-запросы.
//
// Использование:
//   go run loadtest/cmd/nodownload/main.go -url http://localhost:9090 -c 100 -d 30s
//
// Параметры:
//   -url       базовый URL сервера
//   -c         количество конкурентных воркеров
//   -d         длительность теста (например, 30s)
//
// Тест гоняет по тем же эндпоинтам, что и loadtest/cmd/load,
// но использует HEAD-запросы, чтобы сервер выполнял всю логику,
// но не отправлял тело ответа.
...
func main() {
	...
	for i := 0; i < *concurrency; i++ {
		...
		go func(idx int) {
			...
			for {
				...
				// HEAD responses have no body, but close anyway for safety.
				resp.Body.Close()
	...
	fmt.Println()
	fmt.Println("=== HEAD Load Test Results ===")
